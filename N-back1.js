...
let expName = 'N-back1';  // from the Builder filename that created this script
let expInfo = {
    'participant': '',
    'session': '001',
};
...
function przejscieRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of przejscieComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function podsumowanieRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of podsumowanieComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
async function quitPsychoJS(message, isCompleted) {
  ...
  // Prepare data for OSF
  let filename = psychoJS._experiment._experimentName + '_' + psychoJS._experiment._datetime + '.csv';
  
  // Combine calculated metrics into a single object
  let data = {
      hits_nback1: hits_nback1,
      misses_nback1: misses_nback1,
      false_alarms_nback1: false_alarms_nback1,
      correct_rejections_nback1: correct_rejections_nback1,
      accuracy_nback1: accuracy_nback1,
      mean_rt_nback1: mean_rt_nback1,
      hits_nback2: hits_nback2,
      misses_nback2: misses_nback2,
      false_alarms_nback2: false_alarms_nback2,
      correct_rejections_nback2: correct_rejections_nback2,
      accuracy_nback2: accuracy_nback2,
      mean_rt_nback2: mean_rt_nback2,
      hits_total: hits_total,
      misses_total: misses_total,
      false_alarms_total: false_alarms_total,
      correct_rejections_total: correct_rejections_total,
      accuracy_total: accuracy_total,
      mean_rt_total: mean_rt_total,
  };
  
  // Send data to OSF via DataPipe
  console.log('Saving data to OSF...');
  fetch('https://pipe.jspsych.org/api/data', {
      ...
      body: JSON.stringify({
          experimentID: 'EDuQRfBXXb8H', // Replace with your actual DataPipe experiment ID
          filename: filename,
          data: data,
      }),
  })
      ...
      .then(responseData => {
          console.log('Data successfully sent to OSF:', responseData);
          quitPsychoJS();
      })
      .catch(error => {
